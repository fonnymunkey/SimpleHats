...
public class HatJson {
    ...
    public static void registerHatJson() {
        try {
            File file = new File(Services.PLATFORM.getConfigDir(), Constants.MOD_ID + ".json");

            if(!file.exists()) {
                Constants.LOG.info("SimpleHats simplehats.json not found, generating default file.");
                ...
                hatList = defaultHats;
                Constants.LOG.info("Loaded " + hatList.size() + " hat entries from default file.");
            }
            else {
                file.setWritable(true);
                ...
                for(Map.Entry<String, JsonElement> entry : json.entrySet()){
                    ...
                    if(hatEntry.getHatName().isEmpty()) {
                        Constants.LOG.warn("Attempted to load empty hat name, skipping.");
                        continue;
                    }
                    if(!validateName(hatEntry.getHatName())) {
                        Constants.LOG.warn("Attempted to load invalid hat name \"" + hatEntry.getHatName() + "\", skipping.");
                        continue;
                    }
                    for(HatEntry temp : hatList) {
                        if(temp.getHatName().equalsIgnoreCase(hatEntry.getHatName()) || hatEntry.getHatName().equalsIgnoreCase("special")) {
                            Constants.LOG.warn("Attempted to load duplicate hat name \"" + hatEntry.getHatName() + "\", skipping.");
                            continue outer;
                        }
                    }
                ...
                Constants.LOG.info("Loaded " + hatList.size() + " hat entries from simplehats.json");
            }
        }
        catch(Exception ex) {
            Constants.LOG.error("Loading simplehats.json failed: " + ex);
        }
    }
